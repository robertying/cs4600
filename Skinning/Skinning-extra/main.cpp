...
void setJointRotations(float t)
{
	...
	for (unsigned int jointID = 0; jointID < g_numJoints; ++jointID)
	{
		Quaternionf qB = Quaternionf(g_poses[pose][4 * jointID], g_poses[pose][4 * jointID + 1], g_poses[pose][4 * jointID + 2], g_poses[pose][4 * jointID + 3]);
	...
	std::vector<Vector3f>& p_deformedVertices)
{
	...
	for (unsigned int v = 0; v < p_vertices.size(); v++)
	{
		...
		for (unsigned int j = 0; j < p_numJoints; j++) {
			Vector3f p_vertice = fromHomog(p_jointTrans[j] * p_jointTransRestInv[j] * toHomog(p_vertices[v]));
