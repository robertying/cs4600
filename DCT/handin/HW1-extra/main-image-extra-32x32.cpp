...
void processImage(const std::vector<float> I, std::vector<float>& O, int m)
{
	...
	for (int u = 0; u < H; u += 32) {
		for (int v = 0; v < W; v += 32) {
			for (int i = 0; i < 32; i++) {
				for (int j = 0; j < 32; j++) {
					A[i * 32 + j] = I[(u + i)*W + (v + j)];
				}
			}
			processBlock(A, B, m);
			for (int i = 0; i < 32; i++) {
				for (int j = 0; j < 32; j++) {
					O[(u + i)*W + (v + j)] = B[i * 32 + j];
				}
			}
		}
	}
...
int main()
{
	...
	int m = 16;	// TODO: change the parameter m from 1 to 32 to see different image quality
