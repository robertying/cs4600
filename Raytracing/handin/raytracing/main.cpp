	...
	const std::vector<Sphere> &spheres)
{
	...
	for (int j = 0; j < 3; ++j) {
		...
		Vector3f rayDirection2 = lightPositions[j] - hitPoint;
		...
		if (!blocked) {
			...
			N.normalize();
			Vector3f L = lightPositions[j] - hitPoint;
			L.normalize();
			Vector3f V = -rayDirection;
			pixelColor += phong(L, N, V, spheres[sphereIndex].surfaceColor, Vector3f::Ones(), 1.f, 3.f, 100.f) / 3;
		}
	}
